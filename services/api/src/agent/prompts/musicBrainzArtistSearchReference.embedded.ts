/**
 * Default MusicBrainz artist-search reference for the query planner. Kept in sync with
 * `musicbrainz-artist-search.md` (see test). Used when the markdown file is not on disk
 * (e.g. minimal deploy) so the API never depends on loose assets.
 */
export const EMBEDDED_MUSICBRAINZ_ARTIST_SEARCH_REFERENCE = [
