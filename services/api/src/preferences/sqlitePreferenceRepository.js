...
function createSqlitePreferenceRepository({ db }) {
  return {
    ...
    async updateSavedBand(id, updates, userId = DEFAULT_USER) {
      ...
      ).run(next.rating, JSON.stringify(next.categories.map((c) => String(c).trim()).filter(Boolean)), String(next.note).trim(), updatedAt, id, userId);
    ...
    async buildContextForIds(ids, userId = DEFAULT_USER) {
      ...
      const rows = db.prepare(`SELECT * FROM saved_bands WHERE user_id = ? AND id IN (${placeholders})`).all(userId, ...ids);
    ...
    async importSavedBands(bands, userId = DEFAULT_USER) {
      const existingRows = db.prepare("SELECT musicbrainz_artist_id FROM saved_bands WHERE user_id = ?").all(userId);
    ...
    async listGroups(userId = DEFAULT_USER) {
      const rows = db.prepare("SELECT * FROM artist_groups WHERE user_id = ? ORDER BY name ASC").all(userId);
      return rows.map((g) => {
    ...
    async createGroup(name, userId = DEFAULT_USER) {
      ...
      const existing = db.prepare("SELECT id FROM artist_groups WHERE user_id = ? AND name = ?").get(userId, trimmed);
      if (existing) return { ok: false, status: 409, error: "group name already exists" };
      const id = randomUUID();
      const now = new Date().toISOString();
      db.prepare("INSERT INTO artist_groups (id, user_id, name, created_at, updated_at) VALUES (?, ?, ?, ?, ?)").run(id, userId, trimmed, now, now);
      return { ok: true, group: { id, name: trimmed, memberIds: [] } };
    },

    async renameGroup(id, name, userId = DEFAULT_USER) {
      const group = db.prepare("SELECT * FROM artist_groups WHERE id = ? AND user_id = ?").get(id, userId);
      if (!group) return { ok: false, status: 404, error: "group not found" };
      const trimmed = String(name || "").trim();
      if (!trimmed) return { ok: false, status: 400, error: "group name is required" };
      const conflict = db.prepare("SELECT id FROM artist_groups WHERE user_id = ? AND name = ? AND id != ?").get(userId, trimmed, id);
    ...
    async deleteGroup(id, userId = DEFAULT_USER) {
      const result = db.prepare("DELETE FROM artist_groups WHERE id = ? AND user_id = ?").run(id, userId);
    ...
    async addArtistToGroup(groupId, savedBandId, userId = DEFAULT_USER) {
      const group = db.prepare("SELECT id FROM artist_groups WHERE id = ? AND user_id = ?").get(groupId, userId);
      if (!group) return { ok: false, status: 404, error: "group not found" };
      const now = new Date().toISOString();
