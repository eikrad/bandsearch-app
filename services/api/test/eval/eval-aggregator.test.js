...
const {
  aggregateMetrics,
  computeDelta,
} = require("../../src/eval/evalAggregator");
...
test("computeDelta: returns correct difference between two metrics", () => {
  const current = {
    ...
    obscurityDistribution: { cult: 10, underground: 20, obscure: 10 },
  ...
  const baseline = {
    ...
    obscurityDistribution: { cult: 5, underground: 10, obscure: 5 },
...
test("computeDelta: returns null for dimensions where either value is null", () => {
  const current = {
    ...
    obscurityDistribution: { cult: 0, underground: 0, obscure: 0 },
