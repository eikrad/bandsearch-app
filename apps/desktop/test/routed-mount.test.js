...
test("routed mount settingsHandlers.onSaveTursoConfig calls provided saveTursoConfig", async () => {
  ...
  const mount = createDesktopReactMount({
    ...
    createRootImpl: () => ({ render: (el) => renders.push(el) }),
    resolveContainer: () => ({}),
  });

  await mount.mount();
  const rendered = renders[0];
  const settingsHandlers = rendered?.props?.handlers;
  assert.ok(typeof settingsHandlers?.onSaveTursoConfig === "function", "should expose onSaveTursoConfig");

  await settingsHandlers.onSaveTursoConfig("libsql://test.turso.io", "mytoken");
