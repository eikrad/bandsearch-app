...
1. **`createMusicBrainzQueryPlanner`** (`services/api/src/agent/musicBrainzQueryPlanner.ts`): Gemini `gemini-2.5-flash`, low temperature, returns only `{"musicBrainzQuery":"..."}`. Two attempts: normal system prompt, then one retry with a stricter correction addendum.
